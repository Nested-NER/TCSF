...
def select_toi(toi_batch):
    ...
    space_list = []
    cnt = 0
    for sent in toi_batch:
        if len(sent):
            tois.append(np.array([(t[0], t[1]) for t in sent]))
        else:
            space_list.append(cnt)
        cnt += 1
    labels = [np.array([t[2] for t in sent]) for sent in toi_batch]

    return tois, labels, space_list
...
def judge_cover(candidate, entity_list):
    for each in entity_list:
        if(each[0] > candidate[0] and each[1] < candidate[1]):
            return True
        if(each[0] > candidate[0] and each[1] <= candidate[1]):
            return True
        if (each[0] >= candidate[0] and each[1] < candidate[1]):
...
def dfs(idx, gt, gt_layer, depth=3):
    if gt_layer[idx] != -1:
        return gt_layer[idx];
    gt_layer[idx] = -2;
    ...
    for contain_id in range(len(gt)):
        ...
        if gt[contain_id][0] >= gt[idx][0] and gt[contain_id][1] <= gt[idx][1] and gt_layer[contain_id] != -2:
            gt_layer[idx] = min(max(gt_layer[idx], dfs(contain_id, gt, gt_layer) + 1), depth - 1)
