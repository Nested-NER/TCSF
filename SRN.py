...
class SRFN(nn.Module):
    def __init__(self, N, d_model, h, dropout, mix=False):
        super(SRFN, self).__init__()
...
class ForgetConnection(nn.Module):
    """
    A residual connection followed by a layer norm.
    Note for code simplicity the norm is first as opposed to last.
    """
    ...
    def __init__(self, size, dropout):
        super(ForgetConnection, self).__init__()
        self.norm = LayerNorm(size)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x, sublayer):
        ...
        """
        # return x + self.dropout(sublayer(self.norm(x)))
        return self.norm(x * torch.sigmoid(self.dropout(sublayer(x))))
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, feed_forward, dropout):
        ...
        self.forget_sublayer = ForgetConnection(size, dropout)
        self.size = size

    def forward(self, x, mask):
        x = self.forget_sublayer(x, lambda x: self.self_attn(x, x, x, mask))
